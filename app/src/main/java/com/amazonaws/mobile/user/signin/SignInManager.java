...
public class SignInManager {
     ...
     */
    private SignInManager(final Activity activity) {
     ...
     * @return instance
     */
    public synchronized static SignInManager getInstance(final Activity activity) {
        if (singleton == null) {
            singleton = new SignInManager(activity);
        }
        return singleton;
    }
    ...
    public void setDefaultResultsHandler(final Activity activity) {
        final IdentityManager identityManager = AWSMobileClient.defaultMobileClient()
                .getIdentityManager();

        resultsAdapter = new SignInResultsAdapter(activity, new IdentityManager.SignInResultsHandler(){

            @Override
            public void onSuccess(IdentityProvider provider) {
                NSProvider.getSignInHandler().onSuccess(provider);
            }

            @Override
            public void onCancel(IdentityProvider provider) {
                NSProvider.getSignInHandler().onCancel();
            }

            @Override
            public void onError(IdentityProvider provider, Exception ex) {
                NSProvider.getSignInHandler().onFailure(ex);
            }
        } );
        // Set the final results handler with the identity manager.
        identityManager.setResultsHandler(resultsAdapter);
     ...
     */
    public void initialize(final Class<? extends SignInProvider> providerClass) {
        final IdentityManager identityManager = AWSMobileClient.defaultMobileClient()
            .getIdentityManager();
        ...
        if(resultsAdapter != null )
                activity = resultsAdapter.getActivity();
        else if(this.activity != null)
                activity  = this.activity;

        if(activity == null){
            throw new IllegalArgumentException("No activity set. call SignInManager.getInstance(Activity) or SignInManager.setResultsHandler");
        }
